...
Verba 的目标不是证明符号“坏”，而是把结构重新展开成可读单词。begin 与 end 明确表达作用域；call 表达调用；let 表达绑定；with 表达命名参数；result、optional、list 等词语直接表达类型构造。符号仍被保留，但只在它们真正属于的领域中出现。
...
let endpoint url https://api.example.com/v1/users?page=1
let config_path path C:\service\config.json
path /users/{user_id}
let message text user was not found
```
...
output string
begin
    let lowered call lowercase value
    let trimmed call trim lowered
    return trimmed
end
```
...
```verba
let email get request_user profile email
set mutable_user profile display_name new_name
```
...
```verba
let subtotal call multiply unit_price quantity
let total call add subtotal shipping_fee
let is_large call greater_than total 1000
```
...
它们在语义上可以作为编译器内建，也可以在标准库中表现为普通函数。语言不提供可自定义的符号运算符，也不需要运算符优先级表。

## 5.3 条件与循环

```verba
let adult call greater_equal age 18
let verified get user verified
let allowed call and adult verified
...
begin
    ...
    set should_retry call retry_needed
end
```
...
```verba
let message_id try call send_email
begin
...
output string
begin
    let nickname get user_value nickname
...
begin
    let row try call repository_find_user user_id
    let user_value try call map_user_row row
...
let user_row try call sql_one find_user
begin
...
output result user app_error
begin
    let user_id try call path_uuid user_id
    let user_value try call service_find_user user_id
...
let payload try call json_decode create_user_request request_body
let created try call user_service_create payload
...
begin
    let order_row try call sql_one insert_order
    begin
...
let page call render profile_page
begin
...
error VRB1204 at user_service.vrb line 42 column 8
    ...
    let valid call equals email
                   ^^^^^^^^^^^^

equals requires 2 arguments but received 1

suggestion
    let valid call equals email expected_email
```
...
begin
    ...
    let email_valid call regex_match email_pattern email
    let email_invalid call not email_valid
    ...
    let row try call sql_one insert_user
    begin
        with id user_id
        with name name
        with email email
...
begin
    let payload try call json_decode create_user_request request_body
    let created try call create_user payload
    respond json 201 created
end
...
output result user app_error
begin
    let user_id try call parse_uuid id

    let found try call sql_optional find_user
    begin
        with id user_id
    end

    let missing call is_none found
    ...
    let user_value call unwrap found
    respond json 200 user_value
end
```
...
- **条件表达式：**是否允许 if equals a b 作为语法糖，或统一要求 if call equals a b。

- **单行文本：**text 捕获行剩余部分是否会影响自动补全和代码重排。

- **数值符号：**负号和小数点应视为普通数值 token，还是要求类型引导。

- **参数块：**let result call f 后紧接 begin 的语法在大型代码中是否足够清晰。
...
| 作用域 { ... } | begin ... end |
| 赋值 x = value | let x value 或 set x value |
